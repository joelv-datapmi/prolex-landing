...
const plans = [
    {
        ...
        tagline: "Perfect for practitioners.",
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <ScrollReveal delay={0.1}>
                    <div className="flex items-center justify-center gap-4 mb-8">
                        <span className={cn("text-[18px] font-jakarta", billing === "monthly" ? "text-gray-900" : "text-gray-400")}>Monthly</span>
                        <button
                            onClick={() => setBilling(b => b === "monthly" ? "yearly" : "monthly")}
                            className={cn("relative w-[80px] h-[40px] rounded-full transition-colors duration-300", billing === "yearly" ? "bg-[#1a237e]" : "bg-gray-300")}
                        >
                            <m.span
                                layout
                                transition={{ type: "spring", stiffness: 500, damping: 30 }}
                                className={cn("absolute top-[6px] w-[28px] h-[28px] rounded-full bg-white shadow", billing === "yearly" ? "left-[46px]" : "left-[6px]")}
                            />
                        </button>
                        <span className={cn("text-[18px] font-jakarta", billing === "yearly" ? "text-gray-900" : "text-gray-400")}>Yearly</span>
                    </div>
                </ScrollReveal>
                            ...
                            <div className={cn(
                                ...
                                {plan.popular && (
                                    <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                                        <span className="bg-white text-[#1a237e] text-[14px] font-bold px-3 py-1 rounded-full shadow">Most Popular</span>
                                    </div>
                                )}
                                <p className={cn("text-[20px] font-jakarta mb-1", plan.popular ? "text-blue-200" : "text-gray-400")}>{plan.name}</p>
                                <div className="flex items-end gap-1 mb-1">
                                    <AnimatePresence mode="wait">
                                        <m.span
                                            ...
                                            className={cn("font-jakarta text-[48px]", plan.custom ? "text-[48px]" : "text-[48px]", plan.popular ? "text-white" : "text-gray-900")}
                                        >
                                ...
                                <p className={cn("text- mb-5 leading-snug", plan.popular ? "text-blue-100" : "text-gray-500")}>{plan.tagline}</p>
